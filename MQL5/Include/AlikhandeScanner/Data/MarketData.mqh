...
public:
   ...
   bool Snapshot(const string requested,const string symbol,const int max_tick_age_seconds,AS_SymbolSnapshot &s) {
      ...
      s.bid=tick.bid; s.ask=tick.ask; s.tick_time=(datetime)tick.time; s.point=SymbolInfoDouble(symbol,SYMBOL_POINT); s.digits=(int)SymbolInfoInteger(symbol,SYMBOL_DIGITS);
