...
private:
   ...
   bool Exec(const string sql){
      ...
      ResetLastError();
      if(!DatabaseExecute(m_db,sql)){PrintFormat("Alikhande DB error=%d sql=%s",GetLastError(),sql);return false;}
      return true;
   }
public:
   ...
   bool Open(){
      ...
      m_db=DatabaseOpen(AS_DATABASE_FILE,DATABASE_OPEN_READWRITE|DATABASE_OPEN_CREATE);
      if(m_db==INVALID_HANDLE){PrintFormat("Alikhande DB open failed error=%d",GetLastError());return false;}
   ...
   bool Migrate(){
      ...
      string q=StringFormat("INSERT OR REPLACE INTO meta(key,value) VALUES('schema_version','%d')",AS_SCHEMA_VERSION);
      return Exec(q);
   }
