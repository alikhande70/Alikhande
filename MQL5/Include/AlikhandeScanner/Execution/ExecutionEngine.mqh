...
private:
   ...
   bool ScannerDeal(const MqlTradeTransaction &trans){
      if(trans.deal==0 || !HistoryDealSelect(trans.deal)) return false;
      if((ulong)HistoryDealGetInteger(trans.deal,DEAL_MAGIC)!=AS_MAGIC) return false;
      return HistoryDealGetString(trans.deal,DEAL_SYMBOL)==m_current.symbol;
   }
public:
   AS_ExecutionEngine(void){m_repo=NULL;ZeroMemory(m_current);m_current.state=AS_EXEC_IDLE;}
   void Attach(AS_Repositories &repo){m_repo=&repo;m_repo.LoadLatestUnresolvedExecution(m_current);}
   ...
   bool SubmitDemo(const AS_TradePlan &p,string &reason){
      ...
      m_current.execution_id=AS_Fnv1a(p.plan_id+"|"+IntegerToString((int)TimeCurrent()));
      ...
      if(m_repo!=NULL)m_repo.SaveExecution(m_current);
   ...
   void OnTransaction(const MqlTradeTransaction &trans,const MqlTradeRequest &request,const MqlTradeResult &result){
      ...
      if(!related && trans.type==TRADE_TRANSACTION_DEAL_ADD && ScannerDeal(trans))related=true;
      if(!related)return;
      ...
      if(trans.type==TRADE_TRANSACTION_DEAL_ADD && trans.deal>0 && HistoryDealSelect(trans.deal)){
         ...
         if(entry==DEAL_ENTRY_OUT || entry==DEAL_ENTRY_OUT_BY){m_current.state=AS_EXEC_COMPLETED;m_current.terminal=true;}
         else{
            m_current.filled_volume+=trans.volume;
            m_current.state=(m_current.filled_volume+1e-8>=m_current.requested_volume?AS_EXEC_FILLED:AS_EXEC_PARTIALLY_FILLED);
         }
      } else if(trans.type==TRADE_TRANSACTION_POSITION){
         m_current.state=AS_EXEC_POSITION_ACTIVE;
      } else if(trans.type==TRADE_TRANSACTION_ORDER_DELETE && m_current.filled_volume<=0){
         m_current.state=AS_EXEC_RECONCILING;
      }
   ...
   void Reconcile(){
      ...
      for(int i=PositionsTotal()-1;i>=0;i--){
         ...
         if(PositionGetString(POSITION_SYMBOL)==m_current.symbol && (ulong)PositionGetInteger(POSITION_MAGIC)==AS_MAGIC){
            m_current.position_id=(ulong)PositionGetInteger(POSITION_IDENTIFIER);
            m_current.state=AS_EXEC_POSITION_ACTIVE;found=true;break;
         }
      }
      if(!found){
         if(m_current.state==AS_EXEC_POSITION_ACTIVE){m_current.state=AS_EXEC_COMPLETED;m_current.terminal=true;m_current.message="POSITION_NO_LONGER_OPEN";}
         else if(TimeCurrent()-m_current.updated_at>30){m_current.state=AS_EXEC_UNKNOWN;m_current.message="RECONCILIATION_UNRESOLVED";}
         else m_current.state=AS_EXEC_RECONCILING;
      }
      m_current.updated_at=TimeCurrent();
      if(m_repo!=NULL)m_repo.SaveExecution(m_current);
   }
