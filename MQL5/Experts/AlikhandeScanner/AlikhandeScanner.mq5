...
void ScanOne(const int i){
   g_data_ready[i]=false;string sym=g_symbols[i];if(sym=="")return;
   AS_SymbolSpec spec;if(!g_spec_reader.Read(sym,spec))return;
   string previous_spec=g_repo.LatestSpecHash(sym);if(previous_spec=="")g_repo.SaveSpec(spec);else if(previous_spec!=spec.spec_hash){g_repo.SaveSpec(spec);g_repo.RuntimeEvent("WARN","Broker","SPEC_DRIFT",sym+" broker specification changed");}g_cached_spec[i]=spec;
   ...
   ENUM_AS_DATA_STATE ds;if(!g_data.EnsureAllReady(sym,InpMinimumBars,ds))return;g_data_ready[i]=true;
...
void OnTimer(){
   ...
   RenderCurrentTab();ChartRedraw();
}

void OnChartEvent(const int id,const long &lparam,const double &dparam,const string &sparam){
   if(id!=CHARTEVENT_OBJECT_CLICK)return;
   ENUM_AS_TAB tab;if(g_ui.ParseTab(sparam,tab)){g_ui.SetTab(tab);RenderCurrentTab();return;}
   string symbol;
   if(g_ui.ParseOpen(sparam,symbol)){int oi=IndexOfSymbol(symbol);OpenManagedChart(symbol,PERIOD_M15);long cid=FindExistingChart(symbol,PERIOD_M15,InpChartReuseMode==AS_MANAGED_ONLY);if(cid>0&&oi>=0)g_overlay.Draw(cid,g_cached_signal[oi]);return;}
   if(g_ui.ParseDetail(sparam,symbol)){g_selected=IndexOfSymbol(symbol);g_ui.SetTab(AS_TAB_SIGNAL);RenderCurrentTab();return;}
...
void OnTradeTransaction(const MqlTradeTransaction &trans,const MqlTradeRequest &request,const MqlTradeResult &result){
   ...
   if(trans.type==TRADE_TRANSACTION_DEAL_ADD&&trans.deal>0&&HistoryDealSelect(trans.deal)){
      ...
      if(entry==DEAL_ENTRY_OUT||entry==DEAL_ENTRY_OUT_BY){double net=HistoryDealGetDouble(trans.deal,DEAL_PROFIT)+HistoryDealGetDouble(trans.deal,DEAL_COMMISSION)+HistoryDealGetDouble(trans.deal,DEAL_SWAP);g_account_guard.RegisterClosedProfit(net);}
   }
}
