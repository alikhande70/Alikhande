//+------------------------------------------------------------------+
//| ClaudeICTEA — ICT multi-timeframe model (Strategy Tester only)    |
...
int OnInit()
{
   ...
   {
      Print("ClaudeICTEA: BACKTEST ONLY. Refusing live/normal chart.");
      return INIT_FAILED;
   }
   if(_Period != PERIOD_M5)
   {
      Print("ClaudeICTEA: run on M5 (uses D1/H4 internally).");
      return INIT_FAILED;
   }
