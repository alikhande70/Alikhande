...
| | |
|---|---|
| **[`desktop/`](desktop/) — Desktop 2.2.0** | A standalone Windows application. Its own process, window and database; MetaTrader runs headless in the background purely as a quote/execution gateway. The full local suite executes, including the MT5 adapter against a terminal double, exact Demo evidence reconstruction and atomic replay replacement. Status: `STATIC_CANDIDATE`; Windows + real MT5 remains `BLOCKED`. |
