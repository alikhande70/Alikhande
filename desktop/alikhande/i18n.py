...
EN: dict[str, str] = {
    ...
    "unit.ms": "ms",
    "recovery.title": "The previous session",
    "recovery.recovery_first_run": "First run on this machine.",
    "recovery.recovery_clean": "The previous session closed cleanly.",
    "recovery.recovery_crash": "The previous session ended without closing. Nothing was in flight, so no order can be affected.",
    "recovery.recovery_clean_with_unresolved": "The previous session closed while an execution was still unresolved. The submit gate is shut until you clear it on the Execution screen.",
    "recovery.recovery_crash_with_unresolved": "The previous session ended without closing, and an execution was unresolved at the time. The broker may or may not have filled it. The submit gate is shut and stays shut until you inspect the account and clear it on the Execution screen.",
    "notify.session.crash_recovered": "Previous session",
    "notify.link.disconnected": "Broker link lost",
    "notify.link.stalled": "Broker feed frozen",
    "notify.link.degraded": "Broker link degraded",
    "notify.link.restored": "Broker link restored",
    "notify.backup.written": "Backup written",
    "notify.backup.failed": "Backup FAILED",
    "notify.robot.paused": "Robot paused itself",
    "notify.robot.resumed": "Robot resumed",
    "shell.environment": "ENVIRONMENT",
    "env.restart": "The environment will be {environment} the next time this application starts. It is not switched now: changing it mid-session would either mix two environments' records in one database or orphan an in-flight execution in the one being left.",
    "env.demo": "Demo",
...
FA: dict[str, str] = {
    ...
    "unit.ms": "م‌ث",
    "recovery.title": "نشست قبلی",
    "recovery.recovery_first_run": "اولین اجرا روی این دستگاه.",
    "recovery.recovery_clean": "نشست قبلی درست بسته شد.",
    "recovery.recovery_crash": "نشست قبلی بدون بسته‌شدن تمام شد. چیزی در جریان نبود، پس هیچ سفارشی تحت تأثیر نیست.",
    "recovery.recovery_clean_with_unresolved": "نشست قبلی در حالی بسته شد که یک اجرا هنوز حل‌نشده بود. دروازهٔ ارسال بسته است تا وقتی در صفحهٔ اجرا آن را پاک کنید.",
    "recovery.recovery_crash_with_unresolved": "نشست قبلی بدون بسته‌شدن تمام شد و در آن لحظه یک اجرا حل‌نشده بود. ممکن است بروکر آن را پر کرده باشد یا نه. دروازهٔ ارسال بسته است و بسته می‌ماند تا حساب را بررسی و در صفحهٔ اجرا پاکش کنید.",
    "notify.session.crash_recovered": "نشست قبلی",
    "notify.link.disconnected": "اتصال بروکر قطع شد",
    "notify.link.stalled": "خوراک بروکر یخ زد",
    "notify.link.degraded": "اتصال بروکر افت کرد",
    "notify.link.restored": "اتصال بروکر برقرار شد",
    "notify.backup.written": "پشتیبان نوشته شد",
    "notify.backup.failed": "پشتیبان‌گیری شکست خورد",
    "notify.robot.paused": "ربات خودش مکث کرد",
    "notify.robot.resumed": "ربات ادامه داد",
    "shell.environment": "محیط",
    "env.restart": "محیط از اجرای بعدی برنامه {environment} خواهد بود. همین حالا عوض نمی‌شود: تغییر آن وسط نشست یا سوابق دو محیط را در یک پایگاه‌داده قاطی می‌کند یا اجرای در جریان را در محیطی که ترک می‌شود بی‌صاحب می‌گذارد.",
    "env.demo": "دمو",
